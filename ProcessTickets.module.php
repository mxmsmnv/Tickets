...
class ProcessTickets extends Process {
	...
	public function ___executeView(): string {
		...
		if ($this->wire('input')->requestMethod('POST')) {
			...
			try {
				...
				if ($action === 'reply') {
					...
					$tickets->updateTicket($id, $this->wire('user'), [
						'subject' => (string)$this->wire('input')->post('subject'),
					]);
					$this->message($this->_('Subject updated.'));
				} elseif ($action === 'draft') {
		...
		$composerOut .= '<div class="TicketsReplyActions"><div><div class="TicketsReplyAttachmentControl">'
			...
			. '<span class="TicketsReplyAttachmentPreview" data-ticket-attachment-preview aria-hidden="true"><i class="fa fa-file-o"></i></span><span><small data-ticket-attachment-state></small><strong data-ticket-attachment-name></strong><small data-ticket-attachment-size></small></span>'
