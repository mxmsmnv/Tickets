...
class ProcessTickets extends Process {
	...
	public function ___execute(): string {
		...
		return $this->workspace($out . '</tbody></table></div></section>');
	}
	...
	public function ___executeForms(): string {
		...
		if (!$isEditor) {
			...
			foreach ($forms as $item) {
				$out .= '<article class="TicketsFormCard"><header><span class="TicketsBadge" data-color="' . (!empty($item['enabled']) ? 'success' : 'neutral') . '">' . $this->_(!empty($item['enabled']) ? 'Active' : 'Disabled') . '</span><span>' . count($item['fields']) . ' ' . $this->_('fields') . '</span></header><h3>' . $this->e($item['title']) . '</h3><p>' . $this->e($item['description']) . '</p><code>[[tickets-form:' . $this->e($item['name']) . ']]</code><footer><a class="uk-button uk-button-default" href="?id=' . (int)$item['id'] . '">' . $this->_('Edit form') . '</a><a href="' . $this->e(rtrim((string)$tickets->public_path, '/') . '/form/' . $item['name'] . '/') . '" target="_blank" rel="noopener">' . $this->_('Preview') . ' <i class="fa fa-external-link"></i></a></footer></article>';
			}
		...
		$out .= '</div></div><div class="uk-card uk-card-default uk-card-body TicketsFormPreview"><div class="TicketsFormFieldsHead"><div><h2>' . $this->_('Live preview') . '</h2><p>' . $this->_('A safe admin-only preview of the customer form. No ticket is submitted.') . '</p></div></div><div data-tickets-form-preview></div></div></section>';
